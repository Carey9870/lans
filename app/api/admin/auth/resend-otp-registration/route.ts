// app/api/admin/resend-otp-registration/route.ts
...
export async function POST(req: NextRequest) {
  try {
    ...
    const adminRes = await pool.query(
      'SELECT id, phone FROM admins WHERE email = $1 AND is_verified = FALSE',
      [email]
    );
    ...
    const otpRes = await pool.query(
      `SELECT id FROM otps WHERE admin_id = $1 AND type = 'registration' AND expires_at > CURRENT_TIMESTAMP`,
      [adminId]
    );
